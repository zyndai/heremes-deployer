...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Intent-only (single-writer rule): flip the flag and, if the agent is live,
  // re-queue it so the worker redeploys with the token wired in. The worker is
  // the only thing that touches Docker.
  const redeploy = REDEPLOY_FROM.includes(agent.status);
  await prisma.agent.update({
    where: { id },
    data: { personaLinked: true, ...(redeploy ? { status: "queued" } : {}) },
  });
